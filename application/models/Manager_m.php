...
class Manager_m extends CI_Model {
	...
	public function exportUsers(){
		...
		$this->db->select('EMP_BADGE_ID,EMP_NAME,EMP_FNAME,EMP_CNIC,EMP_CONTACT,EMP_EMAIL,EMP_DOB,EMP_DOJ,EMP_ADD,DEPT_NAME,DESG_NAME,ROLE_NAME');
	...
	if($result->num_rows() == $totalCount){
		...
		$updateArray =  array('EMP_BADGE_ID'=> $param['employeeBatchId'],'EMP_NAME'=> $param['employeeName'],'EMP_FNAME'=> $param['fatherName'],'EMP_CNIC'=> $param['cnic'],'EMP_CONTACT'=> $param['contact'],'EMP_EMAIL'=> $param['email'],'EMP_PHOTO'=> "",'EMP_DOB'=>$param['dob'],'EMP_DOJ'=>$param['doj'],'EMP_ADD'=> $param['address'],'EMP_DEPT_ID'=> $param['department'],'EMP_ROLE_ID'=> $param['role'],'EMP_DESG_ID'=> $param['designation'],'EMP_STATUS'=> 1 );
	...
	$result = $this->db->query("SELECT * FROM leave_request WHERE LREQ_EMP_ID = '$emp_id' AND LREQ_STATUS_ID = '$emp_leaveStatus'  OR LREQ_EMP_ID = '$emp_id' AND LREQ_START >='$emp_startDate'  OR LREQ_EMP_ID = '$emp_id' AND LREQ_END >='$emp_endDate' OR LREQ_EMP_ID = '$emp_id' AND LREQ_START >='$emp_endDate' OR LREQ_EMP_ID = '$emp_id' AND LREQ_END >='$emp_startDate' ");
	if($result->num_rows() > 0){
		$arr = $result->result_array();
		return 'exist';
	}
	...
	$result1 = $this->db->query("SELECT * FROM leave_request WHERE LREQ_ID ='$r_id' AND  LREQ_START='$emp_startDate' AND LREQ_END = '$emp_endDate' AND  LREQ_REQ_COMMENTS='$requesterComment' AND LREQ_LTYPE_ID = '$emp_LREQ_LTYPE_ID' AND LREQ_STATUS_ID ='$approverAction' ");
		if($result1->num_rows() > 0){
			...
			if($result2->num_rows() > 0){
                   $updateArray = array('LREQ_LTYPE_ID'=> $param['employeeLType'], 'LREQ_START'=> $param['startDate'],'LREQ_END'=> $param['endDate'],'LREQ_REQ_COMMENTS'=> $param['r_Comment'],'LREQ_APP_ID'=> $approverId,'LREQ_STATUS_ID'=> $param['leaveStatus'],'LREQ_APP_COMMENTS'=> $param['approver_Comment']);
	...
	$result = $this->db->query("SELECT SUM(datediff(LREQ_END, LREQ_START)+1)  AS  Days, LREQ_EMP_ID AS UserId FROM leave_request WHERE LREQ_EMP_ID = '$id' AND LREQ_STATUS_ID = '1' ");
	if($result->num_rows() > 0){
