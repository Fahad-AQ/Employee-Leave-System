					      ...
					      <div class="form-group">
							<div class="row">
								<div class="col-lg-6">
									...
									<input type="hidden" id="aar_leaveType" name="lt" value="1" placeholder="Enter Employee ID"  class="form-control" disabled>
									<input type="hidden" id="aar_leaveStatus" name="ls" value="1" placeholder="Enter Employee ID"  class="form-control" disabled>
									<input type="hidden" id="aar_empId" name="empId"  placeholder="Enter Employee ID"  class="form-control" disabled>
									<input type="number" id="aar_empBatchId" name="batchId"  placeholder="Enter Employee ID"  class="form-control" disabled>
								</div>
								<div class="col-lg-6">
									<label class="col-lg-6 text-bold">Name:</label>
									<input type="text" id="aar_empName" name="name"  placeholder="Enter Name"  class="form-control" disabled>
								</div>
						    </div>
							<div class="row">
								<div class="col-lg-6">
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Department:</label>
											<div class="col-lg-14">
											<input type="text" name="department" class="form-control required_editUser"  id="aar_department" disabled>
											</div>
									</div>
								 </div>
								<div class="col-lg-6">
									...
									<div class="col-lg-14">
										<input type="text" name="designation" class="form-control required_editUser"  id="aar_designation" disabled>
									</div>
									</div>
								</div>
						    </div>
									...
									<div class="col-lg-6">
										<div class="form-group">
											<label class="control-label text-bold col-lg-6">Leave Total:</label>
												<div class="col-lg-14">
												<input type="text" name="leaveTotal" class="form-control"  id="aar_LeaveTotal" disabled>
												</div>
										</div>
									</div>
									<div class="col-lg-6">
										...
										<div class="col-lg-14">
											<input type="text" name="LeaveRemaining" class="form-control"  id="aar_LeaveRemaining" disabled>
										</div>
										</div>
									</div>
								</div>
								...
								<div class="col-lg-6">
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Start Date:</label>
											<div class="col-lg-14">
											<input type="date" id="aar_startDate" name="startDate" pattern="(0[1-9]|1[012])[/](0[1-9]|[12][0-9]|3[01])[/](19|20)\d\d" title="Correct dates allowed ex: 09/22/1993"  placeholder="Enter Start Date"  class="form-control required_addRequest" readonly>
											</div>
									</div>
								 </div>
								<div class="col-lg-6">
									...
									<div class="col-lg-14">
										<input type="date" id="aar_endDate" name="endDate" pattern="(0[1-9]|1[012])[/](0[1-9]|[12][0-9]|3[01])[/](19|20)\d\d" title="Correct dates allowed ex: 09/22/2015"  placeholder="Enter End Date"  class="form-control required_addRequest" readonly>
									</div>
									</div>
								</div>
						    </div>
								...
								<div class="col-md-6 leaveStyle form-horizontal">
									<p class="text-bold">Leave Type:</p>
								    			...
								    			<label class="btn btn-primary" style="width :300px" >
													<input type="radio" id="aar_annualType" value="1" checked="checked" class="aar_LType" name="leaveType" >&nbsp;Annual Leave
												</label>
												 <br />
												<label class="btn bg-success" style="width :300px">
													<input type="radio" id="aar_sickType"   value="2" class="aar_LType" name="leaveType" >&nbsp;Sick Leave&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
												</label>
												 <br />
												<label class="btn btn-info" style="width :300px">
													<input type="radio" id="aar_unpaidType" value="3"  class="aar_LType" name="leaveType" >&nbsp;Unpaid Leave
												</label>
								...
								<div class="col-lg-6">
									...
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Total Days</label>
											<div class="col-lg-14">
												<input id="aar_totalDays"  name="totalDays"  placeholder="Automatically Count days"  class="form-control" disabled>
											</div>
										</div>
						     ...
						     <div class="col-lg-12">
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Requestor Comment</label>
											<div class="col-lg-14">
												<textarea rows="3" cols="3" class="form-control required_addRequest" placeholder="Enter Reason" id="aar_rComment"  name="rComment" ></textarea>
											</div>
										</div>
								</div>
						  </div>
...
<script>
 ...
 $(document).ready(function () { 
	...
	loader.style.display = "block";
	 $( "#aar_empId" ).val("<?php echo $this->session->userdata('id') ?>");
	 $( "#aar_empBatchId" ).val("<?php echo $this->session->userdata('batchId') ?>");
	 $( "#aar_empName" ).val("<?php echo $this->session->userdata('name') ?>");
	 $( "#aar_department" ).val("<?php echo $this->session->userdata('department') ?>");
	 $( "#aar_designation" ).val("<?php echo $this->session->userdata('designation') ?>");
	 $( "#aar_leaveType" ).val("1")
...
//     // Time limits
    ...
    var from_$input = $('#aar_startDate').pickadate({
		format: 'mm/dd/yyyy', 
	...
	var to_$input = $('#aar_endDate').pickadate({
	format: 'mm/dd/yyyy', 
	editable : false , today: '',
	clear: '',
	close: ''
...
from_picker.on('set', function(event) {
  if ( event.select ) {
    to_picker.set('min', from_picker.get('select'));
	    if(($('#aar_startDate').val() == "") && ($('#aar_endDate').val() == "" )){
		 $('#aar_totalDays').val("Select Start Date and End Date");
		}
		else if (($('#aar_startDate').val() == "") && ($('#aar_endDate').val() != "" )){
            $('#aar_totalDays').val("Select Start Date");
		}
		else if (($('#aar_startDate').val() != "") && ($('#aar_endDate').val() == "" )){
            $('#aar_totalDays').val("Select End Date");
		}
		else if (($('#aar_startDate').val() == $('#aar_endDate').val()) && (($('#aar_startDate').val() != "") && ($('#aar_endDate').val() != "" ))){
            $('#aar_totalDays').val("1");
		}

		else{

		 $('#aar_totalDays').val(1+daydiff(parseDate($('#aar_startDate').val()), parseDate($('#aar_endDate').val()))+"");
		}
  }
...
$('#aar_annualType').click(function (){
   ...
   jQuery(function(){
		...
		RadionButtonSelectedValueSet('aar_LType', 1);
	})
});

$('#aar_sickType').click(function (){
$( "#aar_leaveType" ).val('2');
   jQuery(function(){
		function RadionButtonSelectedValueSet(cls, SelectdValue) {
			$('.'+cls).each(function() {
		...
		RadionButtonSelectedValueSet('aar_LType', 2);
	})
});

$('#aar_unpaidType').click(function (){
$( "#aar_leaveType" ).val('3');
   jQuery(function(){
		function RadionButtonSelectedValueSet(cls, SelectdValue) {
			$('.'+cls).each(function() {
		...
		RadionButtonSelectedValueSet('aar_LType', 3);
	})
});
...
$('#newRequestForm').on('submit', function(e){
     e.preventDefault();
	  ...
	  $('.message').slideDown(2500).html("Please Wait");
				if( $( "#aar_startDate" ).val() && $( "#aar_endDate" ).val() &&  $( "#aar_totalDays" ).val() && $( "#aar_rComment" ).val() ){
				
						if($( "#aar_leaveType" ).val() == '1'){
                                if(currentStatusMonth < 12){
								   ...
								   $.ajax({
										...
										success: function (data){
											...
											if(currentStatusMonth >= 12 && (parseInt(totalAnnuals) + 1  - (parseInt(approvedDays) + parseInt($( "#aar_totalDays" ).val())) ) > 0){

												if($( "#aar_totalDays" ).val() > 12){
												...
												$.ajax({
													type: 'POST',
													data: {'employeeId' : $("#aar_empId").val(),'employeeLType' :$( "#aar_leaveType" ).val()
													,'startDate' : DateNewFormat,'endDate' : DateNewFormat2,'r_Comment' : $("#aar_rComment").val() ,'leaveStatus' : $("#aar_leaveStatus").val() ,'userEmail' : '<?php echo $this->session->userdata('email'); ?>', 'roleId' : '<?php echo  $this->session->userdata('roleId'); ?>'  , 'designationId' : '<?php echo  $this->session->userdata('designationId'); ?>'},
                                   ...
                                   $.ajax({
										...
										success: function (data){
											...
											if(currentStatusMonth >= 3 && (parseInt(totalSicks) +1 - (parseInt(approvedDays) + parseInt($( "#aar_totalDays" ).val()) ) ) > 0){
                                                var startDate = $("#aar_startDate").val(); 
												var DateinISO = $.datepicker.parseDate('mm/dd/yy', startDate); 
												var DateNewFormat = $.datepicker.formatDate( "yy-mm-dd", new Date( DateinISO ) );
								   ...
								   $.ajax({
										...
										success: function (data){
											...
											if(currentStatusMonth >= 3 && (parseInt(totalUnpaid) +1  - (parseInt(approvedDays) + parseInt($( "#aar_totalDays" ).val())) ) > 0){
