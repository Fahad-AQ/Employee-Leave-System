					      ...
					      <div class="form-group">
							<div class="row">
								<div class="col-lg-6">
									...
									<input type="hidden" id="uer_leaveType" name="lt" value="1" placeholder="Enter Employee ID"  class="form-control" disabled>
									<input type="hidden" id="uer_leaveStatus" name="ls" value="1" placeholder="Enter Employee ID"  class="form-control" disabled>
									<input type="hidden" id="uer_empId" name="empId"  placeholder="Enter Employee ID"  class="form-control" disabled>
									<input type="hidden" id="uer_r_id" name="r_id"  placeholder="Enter Employee ID"  class="form-control" disabled>
									<input type="number" id="uer_empBatchId" name="batchId"  placeholder="Enter Employee ID"  class="form-control" disabled>
								</div>
								<div class="col-lg-6">
									<label class="col-lg-6 text-bold">Name:</label>
									<input type="text" id="uer_empName" name="name"  placeholder="Enter Name"  class="form-control" disabled>
								</div>
						    </div>
							<div class="row">
								<div class="col-lg-6">
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Department:</label>
											<div class="col-lg-14">
											<input type="text" name="department" class="form-control required_editUser"  id="uer_department" disabled>
											</div>
									</div>
								 </div>
								<div class="col-lg-6">
									...
									<div class="col-lg-14">
										<input type="text" name="designation" class="form-control required_editUser"  id="uer_designation" disabled>
									</div>
									</div>
								</div>
						    </div>
									...
									<div class="col-lg-6">
										<div class="form-group">
											<label class="control-label text-bold col-lg-6">Leave Total:</label>
												<div class="col-lg-14">
												<input type="text" name="leaveTotal" class="form-control"  id="uer_LeaveTotal" disabled>
												</div>
										</div>
									</div>
									<div class="col-lg-6">
										...
										<div class="col-lg-14">
											<input type="text" name="LeaveRemaining" class="form-control"  id="uer_LeaveRemaining" disabled>
										</div>
										</div>
									</div>
								</div>
								...
								<div class="col-lg-6">
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Start Date:</label>
											<div class="col-lg-14">
											<input type="date" id="uer_startDate" name="startDate" pattern="(0[1-9]|1[012])[/](0[1-9]|[12][0-9]|3[01])[/](19|20)\d\d" title="Correct dates allowed ex: 09/22/1993"  placeholder="Enter Start Date"  class="form-control required_editRequest" readonly="true">
											</div>
									</div>
								 </div>
								<div class="col-lg-6">
									...
									<div class="col-lg-14">
										<input type="date" id="uer_endDate" name="endDate" pattern="(0[1-9]|1[012])[/](0[1-9]|[12][0-9]|3[01])[/](19|20)\d\d" title="Correct dates allowed ex: 09/22/2015"  placeholder="Enter End Date"  class="form-control required_editRequest" readonly="true">
									</div>
									</div>
								</div>
						    </div>
								...
								<div class="col-md-6 leaveStyle form-horizontal">
									<p class="text-bold">Leave Type:</p>
								    			...
								    			<label class="btn btn-primary" style="width :300px" >
													<input type="radio" id="uer_annualType" value="1" checked="checked" class="uer_LType" name="leaveType" >&nbsp;Annual Leave
												</label>
												 <br />
												<label class="btn bg-success" style="width :300px">
													<input type="radio" id="uer_sickType"   value="2" class="uer_LType" name="leaveType" >&nbsp;Sick Leave&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
												</label>
												 <br />
												<label class="btn btn-info" style="width :300px">
													<input type="radio" id="uer_unpaidType" value="3"  class="uer_LType" name="leaveType" >&nbsp;Unpaid Leave
												</label>
								...
								<div class="col-lg-6">
									...
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Total Days</label>
											<div class="col-lg-14">
												<input id="uer_totalDays"  name="totalDays"  placeholder="Automatically Count days"  class="form-control" disabled>
											</div>
										</div>
						     ...
						     <div class="col-lg-12">
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Requestor Comment</label>
											<div class="col-lg-14">
												<textarea rows="3" cols="3" class="form-control required_editRequest" placeholder="Enter Reason" id="uer_rComment"  name="rComment" ></textarea>
											</div>
										</div>
								</div>
						  </div>
...
<script>
 ...
 $(document).ready(function () { 
	 $( "#uer_empId" ).val("<?php echo $this->session->userdata('id') ?>");
	 $( "#uer_empBatchId" ).val("<?php echo $this->session->userdata('batchId') ?>");
	 $( "#uer_empName" ).val("<?php echo $this->session->userdata('name') ?>");
	 $( "#uer_department" ).val("<?php echo $this->session->userdata('department') ?>");
	 $( "#uer_designation" ).val("<?php echo $this->session->userdata('designation') ?>");
...
yesterday ;
    ...
    var from_$input = $('#uer_startDate').pickadate({
		format: 'mm/dd/yyyy', 
	...
	var to_$input = $('#uer_endDate').pickadate({
	format: 'mm/dd/yyyy', 
	editable : false , today: '',
	clear: '',
	close: ''
...
from_picker.on('set', function(event) {
  if ( event.select ) {
    to_picker.set('min', from_picker.get('select'));
    to_picker.set('select', $('#uer_endDate').val());
	    if(($('#uer_startDate').val() == "") && ($('#uer_endDate').val() == "" )){
		 $('#uer_totalDays').val("Select Start Date and End Date");
		}
		else if (($('#uer_startDate').val() == "") && ($('#uer_endDate').val() != "" )){
            $('#uer_totalDays').val("Select Start Date");
		}
		else if (($('#uer_startDate').val() != "") && ($('#uer_endDate').val() == "" )){
            $('#uer_totalDays').val("Select End Date");
		}

		else if (($('#uer_startDate').val() == $('#uer_endDate').val()) && (($('#uer_startDate').val() != "") && ($('#uer_endDate').val() != "" ))){
            $('#uer_totalDays').val("1");
		}

		else{

		 $('#uer_totalDays').val(1+daydiff(parseDate($('#uer_startDate').val()), parseDate($('#uer_endDate').val()))+"");
		}
  }
...
$('#uer_annualType').click(function (){
   ...
   jQuery(function(){
		...
		RadionButtonSelectedValueSet('uer_LType', 1);
	})
});

$('#uer_sickType').click(function (){
  $( "#uer_leaveType" ).val('2');
   jQuery(function(){
		function RadionButtonSelectedValueSet(cls, SelectdValue) {
			$('.'+cls).each(function() {
		...
		RadionButtonSelectedValueSet('uer_LType', 2);
	})
});

$('#uer_unpaidType').click(function (){
  $( "#uer_leaveType" ).val('3');
   jQuery(function(){
		function RadionButtonSelectedValueSet(cls, SelectdValue) {
			$('.'+cls).each(function() {
		...
		RadionButtonSelectedValueSet('uer_LType', 3);
	})
});
...
$('#editRequestForm').on('submit', function(e){
	...
	$('.message').slideDown(2500).html("Please Wait");
				if( $( "#uer_startDate" ).val() && $( "#uer_endDate" ).val() &&  $( "#uer_totalDays" ).val() && $( "#uer_rComment" ).val() ){
				
						if($( "#uer_leaveType" ).val() == '1'){
                                ...
                                if(currentStatusMonth < 12){
								   ...
								   $.ajax({
										...
										success: function (data){
											...
											if(currentStatusMonth >= 12 && (parseInt(totalAnnuals) + 1  - (parseInt(approvedDays) + parseInt($( "#uer_totalDays" ).val())) ) > 0){
                                                
												var startDate = $("#uer_startDate").val(); 
												var DateinISO = $.datepicker.parseDate('mm/dd/yy', startDate); 
												var DateNewFormat = $.datepicker.formatDate( "yy-mm-dd", new Date( DateinISO ) );

												var endDate = $("#uer_endDate").val(); 
												...
												$.ajax({
													type: 'POST',
													data: {'r_id': $("#uer_r_id").val(),'employeeId' : $("#uer_empId").val(),'employeeLType' :$( "#uer_leaveType" ).val()
													,'startDate' : DateNewFormat,'endDate' : DateNewFormat2,'r_Comment' : $("#uer_rComment").val() ,'leaveStatus' : $("#uer_leaveStatus").val()},
													url: '<?php echo base_url('user/editRequest');?>',	
													dataType: 'json',
													success: function (data){
										   ...
										   var greaterDays = parseInt(totalAnnuals) - parseInt(approvedDays);
											...
											$('.message').addClass('alert-warning');
											$('.message').removeClass('alert-success');
											$('.message').slideDown(2500).html("You dont have enough Annuals leaves");
									...
									if(currentStatusMonth < 3){
                                       ...
                                       $.ajax({
										...
										success: function (data){
											...
											if(currentStatusMonth >= 3 && (parseInt(totalSicks) + 1 - (parseInt(approvedDays) + parseInt($( "#uer_totalDays" ).val()) ) ) > 0){
                                                var startDate = $("#uer_startDate").val(); 
												var DateinISO = $.datepicker.parseDate('mm/dd/yy', startDate); 
												var DateNewFormat = $.datepicker.formatDate( "yy-mm-dd", new Date( DateinISO ) );
								   ...
								   $.ajax({
										...
										success: function (data){
											...
											if(currentStatusMonth >= 3 && (parseInt(totalUnpaid) + 1  - (parseInt(approvedDays) + parseInt($( "#uer_totalDays" ).val())) ) > 0){
