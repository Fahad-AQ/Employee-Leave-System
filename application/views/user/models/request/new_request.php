					      ...
					      <div class="form-group">
							<div class="row">
								<div class="col-lg-6">
									...
									<input type="hidden" id="uar_leaveType" name="lt" value="1" placeholder="Enter Employee ID"  class="form-control" disabled>
									<input type="hidden" id="uar_leaveStatus" name="ls" value="1" placeholder="Enter Employee ID"  class="form-control" disabled>
									<input type="hidden" id="uar_empId" name="empId"  placeholder="Enter Employee ID"  class="form-control" disabled>
									<input type="number" id="uar_empBatchId" name="batchId"  placeholder="Enter Employee ID"  class="form-control" disabled>
								</div>
								<div class="col-lg-6">
									<label class="col-lg-6 text-bold">Name:</label>
									<input type="text" id="uar_empName" name="name"  placeholder="Enter Name"  class="form-control" disabled>
								</div>
						    </div>
							<div class="row">
								<div class="col-lg-6">
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Department:</label>
											<div class="col-lg-14">
											<input type="text" name="department" class="form-control required_editUser"  id="uar_department" disabled>
											</div>
									</div>
								 </div>
								<div class="col-lg-6">
									...
									<div class="col-lg-14">
										<input type="text" name="designation" class="form-control required_editUser"  id="uar_designation" disabled>
									</div>
									</div>
								</div>
						    </div>
								...
								<div class="col-lg-6">
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Leave Total:</label>
											<div class="col-lg-14">
											<input type="text" name="leaveTotal" class="form-control"  id="uar_LeaveTotal" disabled>
											</div>
									</div>
								 </div>
								<div class="col-lg-6">
									...
									<div class="col-lg-14">
										<input type="text" name="LeaveRemaining" class="form-control"  id="uar_LeaveRemaining" disabled>
									</div>
									</div>
								</div>
						    </div>
								...
								<div class="col-lg-6">
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Start Date:</label>
											<div class="col-lg-14">
											<input type="date" id="uar_startDate" name="startDate" pattern="(0[1-9]|1[012])[/](0[1-9]|[12][0-9]|3[01])[/](19|20)\d\d" title="Correct dates allowed ex: 09/22/1993"  placeholder="Enter Start Date"  class="form-control required_addRequest" readonly>
											</div>
									</div>
								 </div>
								<div class="col-lg-6">
									...
									<div class="col-lg-14">
										<input type="date" id="uar_endDate" name="endDate" pattern="(0[1-9]|1[012])[/](0[1-9]|[12][0-9]|3[01])[/](19|20)\d\d" title="Correct dates allowed ex: 09/22/2015"  placeholder="Enter End Date"  class="form-control required_addRequest" readonly>
									</div>
									</div>
								</div>
						    </div>
								...
								<div class="col-md-6 leaveStyle form-horizontal">
									<p class="text-bold">Leave Type:</p>
								    			...
								    			<label class="btn btn-primary" style="width :300px" >
													<input type="radio" id="uar_annualType" value="1" checked="checked" class="uar_LType" name="leaveType" >&nbsp;Annual Leave
												</label>
												 <br />
												<label class="btn bg-success" style="width :300px">
													<input type="radio" id="uar_sickType"   value="2" class="uar_LType" name="leaveType" >&nbsp;Sick Leave&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
												</label>
												 <br />
												<label class="btn btn-info" style="width :300px">
													<input type="radio" id="uar_unpaidType" value="3"  class="uar_LType" name="leaveType" >&nbsp;Unpaid Leave
												</label>
								...
								<div class="col-lg-6">
									...
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Total Days</label>
											<div class="col-lg-14">
												<input id="uar_totalDays"  name="totalDays"  placeholder="Automatically Count days"  class="form-control" disabled>
											</div>
										</div>
						     ...
						     <div class="col-lg-12">
									<div class="form-group">
										<label class="control-label text-bold col-lg-6">Requestor Comment</label>
											<div class="col-lg-14">
												<textarea rows="3" cols="3" class="form-control required_addRequest" placeholder="Enter Reason" id="uar_rComment"  name="rComment" ></textarea>
											</div>
										</div>
								</div>
						  </div>
...
<script>
 ...
 $(document).ready(function () { 
	...
	loader.style.display = "block";
	 $( "#uar_empId" ).val("<?php echo $this->session->userdata('id') ?>");
	 $( "#uar_empBatchId" ).val("<?php echo $this->session->userdata('batchId') ?>");
	 $( "#uar_empName" ).val("<?php echo $this->session->userdata('name') ?>");
	 $( "#uar_department" ).val("<?php echo $this->session->userdata('department') ?>");
	 $( "#uar_designation" ).val("<?php echo $this->session->userdata('designation') ?>");
	 $( "#uar_leaveType" ).val("1")
function incr_date(date_str){
  ...
  return parts.join("/");
    ...
    var uar_rAnnuals = "<?php echo $this->session->userdata('remaining_annuals') ?>";
	var uar_rSicks = "<?php echo $this->session->userdata('remaining_sicks') ?>";
	var uar_empId = "<?php echo $this->session->userdata('id') ?>";
	if(currentStatusMonth >= 12 ){
			$.ajax({
			type: 'POST',
			data: {'employeeId' : uar_empId},
			...
			success: function (data){
				...
				var approvedDays = (parseInt(uar_rAnnuals) > 0)  ? parseInt(uar_rAnnuals) : 0;
				if(data){
				...
				var annualTotal = document.getElementById('uar_LeaveTotal');
				var annualRemaining = document.getElementById('uar_LeaveRemaining');
	...
	if(currentStatusMonth < 12  && currentStatusMonth > 3){
		$.ajax({
			...
			success: function (data){
				...
				if(currentStatusMonth >= 3){
				totalSicks = 6
				approvedDays = (parseInt(uar_rSicks) > 0)  ? parseInt(uar_rSicks) : 0;
				}
				if(data){
					for(var i=0 ; i<data.length ; i++){
					approvedDays+= parseInt(1+daydiff(parseDate(incr_date(data[i].LREQ_START)),parseDate(incr_date(data[i].LREQ_END))));
				...
				var sickTotal = document.getElementById('uar_LeaveTotal');
				var sickRemaining = document.getElementById('uar_LeaveRemaining');
...
//     // Time limits
    ...
    var from_$input = $('#uar_startDate').pickadate({
		format: 'mm/dd/yyyy', 
	...
	var to_$input = $('#uar_endDate').pickadate({
	format: 'mm/dd/yyyy', 
	editable : false , today: '',
	clear: '',
	close: ''
...
from_picker.on('set', function(event) {
  if ( event.select ) {
    to_picker.set('min', from_picker.get('select'));
	    if(($('#uar_startDate').val() == "") && ($('#uar_endDate').val() == "" )){
		 $('#uar_totalDays').val("Select Start Date and End Date");
		}
		else if (($('#uar_startDate').val() == "") && ($('#uar_endDate').val() != "" )){
            $('#uar_totalDays').val("Select Start Date");
		}
		else if (($('#uar_startDate').val() != "") && ($('#uar_endDate').val() == "" )){
            $('#uar_totalDays').val("Select End Date");
		}
		else if (($('#uar_startDate').val() == $('#uar_endDate').val()) && (($('#uar_startDate').val() != "") && ($('#uar_endDate').val() != "" ))){
            $('#uar_totalDays').val("1");
		}

		else{

		 $('#uar_totalDays').val(1+daydiff(parseDate($('#uar_startDate').val()), parseDate($('#uar_endDate').val()))+"");
		}
  }
...
$('#uar_annualType').click(function (){
   ...
   jQuery(function(){
		...
		RadionButtonSelectedValueSet('uar_LType', 1);
	})
});

$('#uar_sickType').click(function (){
$( "#uar_leaveType" ).val('2');
   jQuery(function(){
		function RadionButtonSelectedValueSet(cls, SelectdValue) {
			$('.'+cls).each(function() {
		...
		RadionButtonSelectedValueSet('uar_LType', 2);
	})
});

$('#uar_unpaidType').click(function (){
$( "#uar_leaveType" ).val('3');
   jQuery(function(){
		function RadionButtonSelectedValueSet(cls, SelectdValue) {
			$('.'+cls).each(function() {
		...
		RadionButtonSelectedValueSet('uar_LType', 3);
	})
});
...
$('#newRequestForm').on('submit', function(e){
	...
	$('.message').slideDown(2500).html("Please Wait");
				if( $( "#uar_startDate" ).val() && $( "#uar_endDate" ).val() &&  $( "#uar_totalDays" ).val() && $( "#uar_rComment" ).val() ){
				
						if($( "#uar_leaveType" ).val() == '1'){
                                if(currentStatusMonth < 12){
								   ...
								   $.ajax({
										...
										success: function (data){
											...
											if(currentStatusMonth >= 12 && (parseInt(totalAnnuals) + 1  - (parseInt(approvedDays) + parseInt($( "#uar_totalDays" ).val())) ) > 0){

												if($( "#uar_totalDays" ).val() > 12){
												...
												$.ajax({
													type: 'POST',
													data: {'employeeId' : $("#uar_empId").val(),'employeeLType' :$( "#uar_leaveType" ).val()
													,'startDate' : DateNewFormat,'endDate' : DateNewFormat2,'r_Comment' : $("#uar_rComment").val() ,'leaveStatus' : $("#uar_leaveStatus").val()  ,'userEmail' : '<?php echo $this->session->userdata('email'); ?>', 'roleId' : '<?php echo  $this->session->userdata('roleId'); ?>'  , 'designationId' : '<?php echo  $this->session->userdata('designationId'); ?>'},
													url: '<?php echo base_url('user/addRequest');?>',	
													dataType: 'json',
													success: function (data){
														if(data.status == 'exist' ){
															...
															setTimeout(function(){
																window.location.href = '<?php base_url('user/requests')?>'; 
															}, 1200)
                                   ...
                                   $.ajax({
										...
										success: function (data){
											...
											if(currentStatusMonth >= 3 && (parseInt(totalSicks) +1 - (parseInt(approvedDays) + parseInt($( "#uar_totalDays" ).val()) ) ) > 0){
                                                var startDate = $("#uar_startDate").val(); 
												var DateinISO = $.datepicker.parseDate('mm/dd/yy', startDate); 
												var DateNewFormat = $.datepicker.formatDate( "yy-mm-dd", new Date( DateinISO ) );
								   ...
								   $.ajax({
										...
										success: function (data){
											...
											if(currentStatusMonth >= 3 && (parseInt(totalUnpaid) +1  - (parseInt(approvedDays) + parseInt($( "#uar_totalDays" ).val())) ) > 0){
